...
function traiterPaiementPayPalCB($donnees, $paypal_config) {
    ...
    try {
        ...
        $data = [
            ...
            'redirect_urls' => [
                'return_url' => 'http://' . $_SERVER['HTTP_HOST'] . '/paiement_cb.php?commande=' . $donnees['commande']['idCommande'] . '&status=success',
                'cancel_url' => 'http://' . $_SERVER['HTTP_HOST'] . '/paiement_cb.php?commande=' . $donnees['commande']['idCommande'] . '&status=cancel'
            ]
        ];
